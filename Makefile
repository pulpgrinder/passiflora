...
win_menu.c: Windows/menu.txt mkmenu.sh
	sh mkmenu.sh Windows/menu.txt $(PROGNAME) win_menu.c

$(BINARY): passiflora.c zipzip.c zipdata.c UI.c menu.c
	mkdir -p $(BINDIR)
...
ios: $(IOS_BINARY) icons ios-bundle

$(IOS_BINARY): passiflora.c zipzip.c zipdata.c UI.c menu.c
ifeq ($(UNAME_S),Darwin)
	mkdir -p $(IOS_BINDIR)
...
iossim: $(SIMOS_BINARY) icons iossim-bundle iossim-run

$(SIMOS_BINARY): passiflora.c zipzip.c zipdata.c UI.c menu.c
ifeq ($(UNAME_S),Darwin)
...
iossim-bundle: $(SIMOS_BINARY) icons
ifeq ($(UNAME_S),Darwin)
...
windows: zipdata.c win_menu.c icons $(WV2_LOADER_H) $(WIN_BINARY)
...
$(WIN_BINARY): passiflora.c zipzip.c zipdata.c UI.c win_menu.c $(WV2_LOADER_H)
	mkdir -p $(WIN_BINDIR)
