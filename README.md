...
| `BUNDLE_ID` | reverse-DNS string | `com.pulpgrinder.HeckinChonker` | The bundle identifier used as the Apple bundle ID (macOS/iOS) and Android `applicationId`. Must be unique — app stores reject `com.example.*`. See the [macOS Build Guide](BUILD-macOS.md) for rules. |
