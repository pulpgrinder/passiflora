 ...
 * embedded ZIP archive.  The ZIP is linked in as a binary blob
 * via #include "generated/zipdata.h" (the ZIP bytes themselves live in
 * generated/archive.zip and are linked from generated/zipdata.S with
 * .incbin, so the C compiler never parses the archive as source).
 * The server has no filesystem access whatsoever — all content comes
 ...
 *
 * Build:
 *   ./nixscripts/mkzipfile.sh content_dir generated/zipdata.h
 *   cc -O2 -o passiflora passiflora.c zipdata.o
 *
 * Usage:
...
static void handle_request(int fd)
{
    ...
    if (strcmp(path, "/debug") == 0 || strcmp(path, "/debug/") == 0) {
        static const char debugger_html[] =
            ...
            "<script>\n"
            "let _nonce=0;\n"
            "function s2b(s){const b=[];for(let i=0;i<s.length;i++){const c=s.charCodeAt(i);\n"
            ...
            "  b=b.slice();const bl=b.length*8;b.push(128);\n"
            "  while(b.length%64!==56)b.push(0);\n"
            "  for(let s=56;s>=0;s-=8)b.push((bl/Math.pow(2,s))&255);\n"
            "  const H=[0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19];\n"
            "  for(let o=0;o<b.length;o+=64){const W=[];\n"
            "    for(let t=0;t<16;t++)W[t]=(b[o+t*4]<<24)|(b[o+t*4+1]<<16)|(b[o+t*4+2]<<8)|b[o+t*4+3];\n"
            "    for(let t=16;t<64;t++){const s0=rr(W[t-15],7)^rr(W[t-15],18)^(W[t-15]>>>3);\n"
            "      const s1=rr(W[t-2],17)^rr(W[t-2],19)^(W[t-2]>>>10);W[t]=(W[t-16]+s0+W[t-7]+s1)|0;}\n"
            "    let a=H[0],B=H[1],c=H[2],d=H[3],e=H[4],f=H[5],g=H[6],h=H[7];\n"
            "    for(let t=0;t<64;t++){const S1=rr(e,6)^rr(e,11)^rr(e,25),ch=(e&f)^(~e&g),t1=(h+S1+ch+K[t]+W[t])|0;\n"
            "      const S0=rr(a,2)^rr(a,13)^rr(a,22),mj=(a&B)^(a&c)^(B&c),t2=(S0+mj)|0;\n"
            "      h=g;g=f;f=e;e=(d+t1)|0;d=c;c=B;B=a;a=(t1+t2)|0;}\n"
            "    H[0]=(H[0]+a)|0;H[1]=(H[1]+B)|0;H[2]=(H[2]+c)|0;H[3]=(H[3]+d)|0;\n"
            "    H[4]=(H[4]+e)|0;H[5]=(H[5]+f)|0;H[6]=(H[6]+g)|0;H[7]=(H[7]+h)|0;}\n"
            "  const out=[];for(let i=0;i<8;i++){const v=H[i]>>>0;out.push((v>>24)&255,(v>>16)&255,(v>>8)&255,v&255);}\n"
            "  return out;}\n"
            "function hmac(key,msg){\n"
            "  let kb=s2b(key);if(kb.length>64)kb=sha256(kb);\n"
            "  while(kb.length<64)kb.push(0);\n"
            "  const ip=[],op=[];for(let i=0;i<64;i++){ip.push(kb[i]^0x36);op.push(kb[i]^0x5c);}\n"
            "  const inner=sha256(ip.concat(s2b(msg)));\n"
            "  const outer=sha256(op.concat(inner));\n"
            "  let hex='';for(let i=0;i<outer.length;i++)hex+=('0'+outer[i].toString(16)).slice(-2);\n"
            "  return hex;}\n"
            "function sendDebug(){\n"
            "  const s=document.getElementById('status'),k=document.getElementById('key').value,\n"
            ...
            "      let att=0;const mx=20,iv=setInterval(function(){\n"
            "        att++;fetch('/__passiflora/debug_result').then(function(rr){\n"
            "          if(rr.status===200){clearInterval(iv);rr.json().then(function(j){const p=[];\n"
